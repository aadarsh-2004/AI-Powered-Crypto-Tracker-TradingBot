...
const Dashboard = () => {
    ...
    const sendEmailToBackend = async () => {
      try {
        await sendNotification(
          "+917877571101",
          `Welcome to Crypto Tracker! The logged-in email is: ${user.email}`
        );
      } catch (error) {
  ...
  return (
    <div className="bg-gradient-to-tr from-blue-950 via-gray-950 to-black text-white min-h-screen p-4 sm:p-8">
      {/* Header */}
      <header className="flex flex-row sm:flex-row justify-between items-center mb-8 gap-4">
        <h1 className="text-3xl sm:text-4xl font-medium text-white-400 text-center sm:text-left">
          Crypto Tracker
        </h1>
      ...
      <div className="mb-8">
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-8">
          {/* Top Gainers */}
          <div className="sm:w-1/2 w-full bg-darkBg p-4 rounded-lg">
            <h3 className="text-xl font-medium mb-4 text-green-500">
              Top Gainers
            </h3>
            <div className="grid grid-cols-3 sm:grid-cols-3 gap-4 scrollbar-hide overflow-auto">
              {topGainers.map((coin, index) => (
                ...
                >
                  <h4 className="text-sm sm:text-lg font-bold text-green-200">
                    {coin.name}
                  </h4>
          ...
          <div className="sm:w-1/2 w-full bg-darkBg p-4 rounded-lg">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-3 gap-4 scrollbar-hide overflow-auto">
              {topLosers.map((coin, index) => (
                ...
                >
                  <h4 className="text-sm sm:text-lg font-bold text-red-300">
                    {coin.name}
                  </h4>
                  <p className="text-sm text-white mt-1">{coin.price}</p>
      ...
      <div className="flex flex-col lg:flex-row justify-around gap-8">
        {/* Crypto Table */}
        <div className="w-full lg:w-[830px] scrollbar-hide overflow-auto">
          <CryptoTable onCryptoSelect={handleCryptoSelect} />
        </div>

        {/* Chart Section */}
        <div
          className={`flex flex-col sm:flex-col items-end sm:items-end rounded-xl drop-shadow-[0px_0px_20px_rgba(59,130,246,0.6)] bg-transparent ${
            fullView ? "w-full h-fit" : "w-full sm:w-[600px]"
      ...
      <div className="flex flex-col sm:flex-row mt-8 gap-4">
        {/* News List */}
        <div className={`w-full ${selectedArticle ? "hidden" : ""}`}>
        ...
        {selectedArticle && (
          <div className="w-full bg-cardBg p-6 rounded-lg shadow-lg">
            ...
            <img
              ...
              alt="News"
              className="w-full sm:w-60 h-60 object-cover rounded-lg mb-4"
            />
